...
public class FirstPersonAIO : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        _input = InputController.Singleton;
    }
    ...
    private void Update()
    {
        ...
        if (enableCameraMovement && !controllerPauseState)
        {
            ...
            {
                mouseYInput = mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.X ? _input.MouseDelta.y : -_input.MouseDelta.y;
                mouseXInput = mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.Y ? _input.MouseDelta.x : -_input.MouseDelta.x;
            }
            else
            {
                mouseXInput = _input.Direction.x * (mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.Y ? 1 : -1);
            }
        ...
        if (canHoldJump ? (canJump && _input.IsJump) : (_input.IsJumpDown && canJump))
        {
            jumpInput = true;
        }
        else if (_input.IsJumpUp)
        {
            jumpInput = false;
        }
        ...
        if (_crouchModifiers.useCrouch)
        {
            ...
            {
                isCrouching = _crouchModifiers.crouchOverride || _input.IsCrouch;
            }
            else if (_input.IsCrouchDown)
            {
                isCrouching = !isCrouching || _crouchModifiers.crouchOverride;
            }
        }

        if (_input.IsToMenuDown || _input.IsToGameDown)
        {
            ControllerPause();
        }
        #endregion
    ...
    private void FixedUpdate()
    {
        ...
        if (useStamina)
        {
            isSprinting = _input.IsSprint && !isCrouching && staminaInternal > 0 && (Mathf.Abs(fps_Rigidbody.velocity.x) > 0.01f || Mathf.Abs(fps_Rigidbody.velocity.z) > 0.01f);
            if (isSprinting)
            {
            ...
            else if ((!_input.IsSprint || Mathf.Abs(fps_Rigidbody.velocity.x) < 0.01f || Mathf.Abs(fps_Rigidbody.velocity.z) < 0.01f || isCrouching) && staminaInternal < staminaLevel)
            {
